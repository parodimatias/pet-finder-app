...
const userController = {
    ...
    try {
      ...
      if (created) {
        ...
        await userCreated.createAuth({
          password: this.stringToHash(user.password),
        });
        return "ok";
      } else {
  ...
  async getToken(user: { email; password }) {
    const userFound = await User.findOne({
      ...
      include: [
        {
          model: Auth,
          where: { password: this.stringToHash(user.password) },
        },
      ],
    });
    if (userFound) {
  ...
  async updateProfile(user: { id; newName; newPassword }) {
    try {
      const foundUser = await this.findUserbyPk(user.id);
      if (foundUser) {
        if (user.newName) {
        ...
        if (user.newPassword) {
          ...
          await auth.update({ password: this.stringToHash(user.newPassword) });
        }
  ...
  async reportLostPet(data: { userId; nombre; picture; lat; lng; location }) {
    const foundUser = await this.findUserbyPk(data.userId);
    if (foundUser) {
  ...
  async sendNotification({ petId, reporterName, reporterPhone, reporterDesc }) {
